...
def _worker_pipeline(
    ...
    for a in hw_archs:
        hw = hw_score_map.get(a.arch_id, 0.0)
        refined, alog = tr_refiner.refine(a, hw_score=hw)
        train_map[refined.arch_id] = alog.train_score_end
        tr_archs.append(refined)

    # Sort by preliminary combined (50/50) bukan training saja
    tr_archs.sort(
        key=lambda a: (
            hw_score_map.get(a.arch_id, 0.0) * 0.5 +
    ...
    return {
        ...
        "n_generated":   len(tr_archs),
        "archs":         [_arch_to_dict(a) for a in tr_archs],
...
def stage_parallel(cfg: PipelineConfig):
    ...
    """
    console.rule("[bold cyan]  Stage 1–3 — Parallel Generate + HW-NAS + Training-NAS  ")
    ...
    if RICH:
        ...
        console.print(
            f"[dim]  Tiap worker: Generate → HW-NAS 7D → Training-NAS 6D  "
            f"(seed={cfg.seed}+worker_idx)[/dim]\n"
        )
    else:
    ...
    for idx in sorted(worker_results.keys()):
        ...
        all_train.update(res["train_map"])

    elapsed = time.perf_counter() - t0
    n_ok = len(worker_results)

    console.print(
        f"\n[bold cyan]✓ Parallel Stage 1–3 selesai — "
    ...
    return all_archs, all_hw_scores, all_quality, all_train
...
def stage_balanced_scoring(
    ...
    # Tier 1: fits_gpu + kedua skor balanced (hw DAN train ≥ threshold)
    tier1 = [
    ...
    # Tier 2: fits_gpu saja (jika tidak ada yang balanced)
