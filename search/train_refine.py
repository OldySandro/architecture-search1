...
PROXY_VOCAB       = 256
PROXY_BATCH       = 2
PROXY_TRAIN_STEPS = 50    # BASE — di-override oleh _adaptive_steps() per-arc
PROXY_WARMUP_STEPS = 5    # BASE — di-override oleh _adaptive_steps() per-arc
PROXY_LR          = 3e-3
PROXY_WEIGHT_DECAY = 0.01
PROXY_GRAD_CLIP   = 1.0
PROXY_TAIL_STEPS  = 15    # BASE — proporsional ke steps aktual di compute_derived
MAX_VALID_LOSS    = 8.0   # log(256) ≈ max; lebih = diverged

# Early stopping config
EARLY_STOP_PATIENCE  = 8    # steps tanpa improvement sebelum stop
EARLY_STOP_MIN_DELTA = 0.001  # minimum improvement yang dianggap meaningful
EARLY_STOP_MIN_STEPS = 30   # minimum steps sebelum early stop aktif (naik dari 25)

# Adaptive steps bounds
ADAPTIVE_STEPS_MIN             = 40
ADAPTIVE_STEPS_MAX             = 150   # 120→150: model besar H200 butuh ruang lebih
ADAPTIVE_STEPS_BASE_STEPS      = 50
ADAPTIVE_STEPS_BASE_COMPLEXITY = 12 * (512 ** 0.5)  # L=12, H=512 → ≈271
...
class ProxyTrainer:
    ...
    def _lr_warmup(self, step: int, warmup: int, base_lr: float,
                   total_steps: int = PROXY_TRAIN_STEPS) -> float:
        if step < warmup:
            return base_lr * (step + 1) / max(1, warmup)
        # Cosine decay setelah warmup — pakai total_steps aktual bukan konstanta
    ...
    def _adaptive_steps(self, cfg: ArchConfig) -> Tuple[int, int, int]:
        """
        Hitung jumlah proxy training steps secara adaptif berdasarkan
        kompleksitas arsitektur — bukan angka magic yang sama untuk semua.

        Formula:
          complexity = num_layers × sqrt(hidden_dim)
          steps = BASE_STEPS × sqrt(complexity / BASE_COMPLEXITY)

        Rasionale:
          - Model kecil (Nano, L=6, H=256) konvergen dalam ~35-40 steps
            → lebih dari itu = overfit T4
          - Model besar (CoT H200, L=36, H=2688) perlu ~80+ steps
            → kurang dari itu = T1 masih naik, belum discriminative

        Kenapa sqrt bukan linear:
          - Diminishing return: model 4× lebih kompleks tidak perlu 4× steps
          - sqrt memberi kurva yang lebih natural dan tidak over-run model besar

        Returns:
            (max_steps, warmup_steps, tail_steps)
        """
        complexity = cfg.num_layers * math.sqrt(max(1, cfg.hidden_dim))
        scale      = math.sqrt(max(0.1, complexity / ADAPTIVE_STEPS_BASE_COMPLEXITY))
        max_steps  = int(ADAPTIVE_STEPS_BASE_STEPS * scale)
        max_steps  = int(np.clip(max_steps, ADAPTIVE_STEPS_MIN, ADAPTIVE_STEPS_MAX))

        # Warmup: 10% dari steps, minimum 4, maksimum 15
        warmup = int(np.clip(max_steps * 0.10, 4, 15))

        # Tail: 20% dari steps untuk variance measurement
        tail   = max(8, int(max_steps * 0.20))

        return max_steps, warmup, tail
    ...
    def train(self, cfg: ArchConfig) -> ProxyTrainingResult:
        ...
        Training procedure:
          ...
          2. Generate STRUCTURED data (bigram+shift+modulo)
          3. Split menjadi train (80%) dan val (20%)
          4. Train dengan ADAPTIVE STEPS + EARLY STOPPING
             - Adaptive steps: proporsional ke complexity arc (40–120 steps)
             - Early stopping: berhenti saat loss plateau (tidak buang compute)
          5. Evaluasi val loss setiap 3 steps
          6. Compute derived metrics dengan tail proporsional ke steps aktual

        Kenapa adaptive steps + early stopping:
          - Fixed steps → bias ke family tertentu (kecil overfit, besar underfitting)
          - Adaptive → model kecil cukup 40 steps, model besar sampai 120
          - Early stop → tidak overfit T4 pada structured data yang berulang
        """
        ...
        # ── Adaptive steps berdasarkan kompleksitas arc ───────────────────────
        max_steps, warmup_steps, tail_steps = self._adaptive_steps(cfg)
        ...
        try:
            ...
            # Generate STRUCTURED data — cukup untuk max_steps
            total_tokens_needed = PROXY_SEQ_LEN * PROXY_BATCH * max_steps
            ...
            # Label smoothing — scale dengan depth untuk kurangi T4 bias
            # Model lebih dalam → lebih rawan overfit structured data → smooth lebih
            if cfg.num_layers > 30:
                label_smooth = 0.08
            elif cfg.num_layers > 20:
                label_smooth = 0.05
            else:
                label_smooth = 0.0
            ...
            # ── Training loop dengan adaptive steps + early stopping ──────────
            model.train()
            for step in range(max_steps):
                # LR schedule — cosine dari adaptive_lr dengan warmup adaptif
                lr = self._lr_warmup(step, warmup_steps, adaptive_lr,
                ...
                # Hanya aktif setelah MIN_STEPS dan setelah warmup selesai
                if step >= max(EARLY_STOP_MIN_STEPS, warmup_steps + 5):
                    ...
                    if plateau_count >= EARLY_STOP_PATIENCE:
                        # Loss sudah plateau — berhenti, tidak perlu overfit lebih
                        break
                else:
                    # Dalam warmup atau terlalu awal — update best_loss saja
                    if loss_val < best_loss:
                        best_loss = loss_val

                # Val evaluation setiap 3 steps
                if step % 3 == 2 or step == max_steps - 1:
                    model.eval()
                    with torch.no_grad():
        ...
        # Compute derived metrics dengan steps & tail proporsional ke steps aktual
        # (bukan hardcode PROXY_TRAIN_STEPS yang sudah tidak dipakai)
        actual_steps = len(result.train_losses)
        actual_tail  = max(5, int(actual_steps * 0.20))
        result.compute_derived(actual_steps, actual_tail)
...
class TrainingDynamicsEvaluator:
    ...
    def _score_convergence(self, cfg: ArchConfig, proxy: ProxyTrainingResult) -> float:
        ...
        # Scoring bertingkat (recalibrated untuk 80 steps structured data)
        # Dengan 80 steps, arch bagus bisa mencapai 35-60%+ reduction
        if rel_reduction >= 0.45:            # ≥45%: excellent convergence
            rate_score = 1.00
        elif rel_reduction >= 0.30:          # 30–45%: very good
            rate_score = 0.85 + (rel_reduction - 0.30) / 0.15 * 0.15
        elif rel_reduction >= 0.18:          # 18–30%: good
            rate_score = 0.65 + (rel_reduction - 0.18) / 0.12 * 0.20
        elif rel_reduction >= 0.08:          # 8–18%: acceptable
            rate_score = 0.40 + (rel_reduction - 0.08) / 0.10 * 0.25
    ...
    def _score_stability(self, cfg: ArchConfig, proxy: ProxyTrainingResult) -> float:
        ...
        # Penalti: gradient checkpointing tidak perlu
        # FIX: threshold diturunkan dari 65% ke 50% — banyak arc valid pakai GC
        # untuk margin keamanan meski VRAM masih cukup. 50% lebih fair.
        # Hanya penalize jika VRAM benar-benar rendah (< 50%) DAN pakai GC.
        gc_pen = -0.02 if (cfg.use_gradient_checkpointing and
                           cfg.vram_usage_pct < 50) else 0.0
    ...
    def _score_gradient_health(
        ...
        # FIX: pakai model EXPONENTIAL bukan linear.
        # Linear: 1 - L×0.015 sangat over-penalize model dalam (L=36 → 0.46)
        # Exponential: (1-r)^L lebih realistis — RMSNorm/FlashAttn mengurangi r
        # Sama seperti formula di training_aware.py._gradient_attenuation_factor()
        norm_val  = getattr(cfg, 'norm_type', None)
        norm_str  = norm_val.value if hasattr(norm_val, 'value') else str(norm_val)
        base_rate = GRAD_ATTENUATION_PER_LAYER   # 0.015
        if "RMS" in norm_str.upper():
            base_rate *= 0.60   # RMSNorm: -40% attenuation
        else:
            base_rate *= 0.75   # LayerNorm: -25% attenuation
        if cfg.use_flash_attn:
            base_rate *= 0.90   # Flash attn: numerically stable backward
        if cfg.dropout == 0.0:
            base_rate *= 0.95   # No dropout: gradient lebih bersih
        atten_score = (1.0 - base_rate) ** cfg.num_layers
