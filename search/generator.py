...
class ArchitectureGenerator:
    ...
    # ═════════════════════════════════════════════════════════════════════════
    #  BUDGET SAMPLING — Clamp layers & seq setelah semua param diketahui
    # ═════════════════════════════════════════════════════════════════════════
    ...
    def _budget_clamp(
        ...
        """
        Budget Sampling: clamp num_layers dan seq_len supaya kombinasi
        keduanya tidak meledak VRAM, menggunakan formula yang sama dengan
        _compute_memory() tapi lebih cepat (tanpa buat ArchConfig dulu).

        Dipanggil SETELAH semua parameter lain disampling — jadi tahu
        persis berapa VRAM yang tersedia untuk layers dan seq.

        Returns (num_layers_clamped, seq_len_clamped).
        """
        gpu        = self.gpu
        budget_gb  = gpu.vram_gb * VRAM_LIMIT_PCT * 0.98  # 2% buffer anti-rounding
        DTYPE      = 2   # BF16 bytes

        # ── [1] VRAM per layer (weights + optimizer) ─────────────────────────
        # Params attn per layer
        attn_p = ((num_heads    * head_dim * hidden_dim) +   # Q
                  (num_kv_heads * head_dim * hidden_dim) +   # K
                  (num_kv_heads * head_dim * hidden_dim) +   # V
                  (num_heads    * head_dim * hidden_dim))    # O

        ffn_dim = int(hidden_dim * ffn_mult)
        if ffn_type in (FFNType.GEGLU, FFNType.GATED):
            ffn_p = hidden_dim * ffn_dim * 2 + ffn_dim * hidden_dim
        elif ffn_type in (FFNType.MOE, FFNType.MOE_TOPK):
            ffn_p = num_experts * (hidden_dim * ffn_dim * 2 + ffn_dim * hidden_dim)
        else:
            ffn_p = hidden_dim * ffn_dim + ffn_dim * hidden_dim

        norm_p       = 4 * hidden_dim
        params_layer = attn_p + ffn_p + norm_p

        # Optimizer bytes per param
        _opt_bpp = {
            OptimizerType.ADAM_FP32:  12.0,
            OptimizerType.ADAM_8BIT:   6.0,
            OptimizerType.LION:        8.0,
            OptimizerType.ADAMW_BF16:  6.0,
            OptimizerType.ZERO1:      12.0,
            OptimizerType.ZERO2:      10.0,
            OptimizerType.ZERO3:       8.0,
        }.get(opt_type, 12.0)

        # VRAM weights + optimizer per layer (GB)
        layer_wo_gb = (params_layer * (DTYPE + _opt_bpp)) / 1e9

        # ── [2] Fixed VRAM (embedding — tidak tergantung L) ──────────────────
        embed_p    = 32000 * hidden_dim   # vocab * hidden
        fixed_gb   = (embed_p * (DTYPE + _opt_bpp)) / 1e9

        # ── [3] Hitung max_layers dari weight+optimizer budget ────────────────
        # Sisakan 35% budget untuk activations + KV cache
        wo_budget  = budget_gb * 0.65 - fixed_gb
        if layer_wo_gb > 0:
            max_layers_wo = int(wo_budget / layer_wo_gb)
        else:
            max_layers_wo = l_hi

        # ── [4] Activations + KV per layer per token ─────────────────────────
        # Activation per layer (simplified _compute_memory formula)
        # attn: prenorm + Q + K + V + O
        act_per_layer_base = (
            batch_size * hidden_dim * DTYPE    +   # prenorm
            batch_size * num_heads    * head_dim * DTYPE +  # Q
            batch_size * num_kv_heads * head_dim * DTYPE +  # K
            batch_size * num_kv_heads * head_dim * DTYPE +  # V
            batch_size * hidden_dim * DTYPE        # O
        )  # per token (masih perlu × seq_len)

        if ffn_type in (FFNType.GEGLU, FFNType.GATED):
            act_ffn_base = batch_size * ffn_dim * DTYPE * 3
        elif ffn_type in (FFNType.MOE, FFNType.MOE_TOPK):
            act_ffn_base = (batch_size * ffn_dim * DTYPE * top_k +
                            batch_size * num_experts * 4)
        else:
            act_ffn_base = batch_size * ffn_dim * DTYPE * 2
        ...
        # FIX 1: Hapus × 1.50 backward multiplier — GC sudah handle ini,
        # double count menyebabkan layer di-clamp terlalu agresif.
        # Tambah 1.10 saja sebagai overhead residual stream & norm.
        act_per_token_per_layer = (act_per_layer_base + act_ffn_base) * 1.10

        # KV cache per layer per token
        kv_per_token_per_layer = 2 * num_kv_heads * head_dim * batch_size * DTYPE

        # FIX 2: GC factor pakai l_lo (bukan num_layers yang belum di-clamp)
        # Pakai l_lo supaya estimasi konservatif tapi realistis
        if use_gc:
            _gc_ref = max(l_lo, 4)
            gc_factor = max(0.15, 1.0 / max(1, int(math.sqrt(_gc_ref))))
        else:
            gc_factor = 1.0

        # FIX 3: Naikkan budget aktivasi dari 35% → 50%
        # Mayoritas VRAM model kecil (< 1B) justru di aktivasi, bukan weights
        act_kv_budget = budget_gb * 0.50

        # ── [5] Untuk setiap seq candidate: hitung max_layers ────────────────
        best_seq     = seq_candidates[0]
        best_layers  = l_lo
        best_score   = 0.0   # makin besar layers × seq makin bagus

        for s in seq_candidates:
            # Total act+KV per layer untuk seq ini
            act_layer_gb = (act_per_token_per_layer * s * gc_factor) / 1e9
            kv_layer_gb  = (kv_per_token_per_layer  * s) / 1e9
            total_per_layer = act_layer_gb + kv_layer_gb

            if total_per_layer <= 0:
                continue

            max_layers_act = int(act_kv_budget / total_per_layer)
            raw_max = min(l_hi, max_layers_wo, max_layers_act)

            # FIX: jika budget tidak cukup untuk l_lo layer → seq ini infeasible
            # Jangan pakai l_lo sebagai floor di scoring — itu menyesatkan
            if raw_max < l_lo:
                continue   # skip seq ini, tidak bisa fit minimum layers

            # Score: prefer arc yang layers × seq besar (lebih ekspresif)
            score = raw_max * s
            if score > best_score:
                best_score  = score
                best_seq    = s
        ...
        # ── [6] Sample dalam range valid ─────────────────────────────────────
        if best_score > 0.0:
            # Ada kombinasi feasible → sample dalam range
            clamped_layers = rng.randint(l_lo, max(l_lo, best_layers))
            clamped_seq    = best_seq
        else:
            # Tidak ada kombinasi feasible — budget terlalu kecil untuk l_lo
            # Cari jumlah layer maksimum yang BISA muat, walau di bawah l_lo
            fallback_layers = max(4, l_lo)
            fallback_seq    = seq_candidates[0]   # seq terkecil
            for s in seq_candidates:
                act_layer_gb    = (act_per_token_per_layer * s * gc_factor) / 1e9
                kv_layer_gb     = (kv_per_token_per_layer  * s) / 1e9
                total_per_layer = act_layer_gb + kv_layer_gb
                if total_per_layer <= 0:
                    continue
                max_layers_act = int(act_kv_budget / total_per_layer)
                candidate_l    = max(4, min(l_hi, max_layers_wo, max_layers_act))
                if candidate_l >= fallback_layers or s == seq_candidates[0]:
                    fallback_layers = candidate_l
                    fallback_seq    = s
                    break   # seq_candidates diurutkan kecil → besar, ambil pertama yang fit
            clamped_layers = fallback_layers
            clamped_seq    = fallback_seq

        return clamped_layers, clamped_seq
    ...
    def generate_one(self, family_name: str) -> ArchConfig:
        ...
        # Formula konservatif: hitung weight(2B) + optimizer(10B avg) = 12B/param
        # Param per layer ≈ 12 × D² (attn + FFN tipikal)
        # Budget 55% VRAM untuk weights + optimizer:
        # 12 × D² × L_min × 12B ≤ 0.55 × vram_gb × 1e9
        # → D_max = sqrt(0.55 × vram_gb × 1e9 / (144 × L_min))
        _vram_gb_safe   = gpu.vram_gb * VRAM_LIMIT_PCT
        _max_hidden_dim = self._make_divisible(
            int(math.sqrt(max(1, _vram_gb_safe * 0.55 * 1e9 /
                              (144.0 * max(1, _l_lo))))), 128)
        ...
        # note: seq_len akan di-override oleh _budget_clamp() di bawah
        ...
        # ── BUDGET SAMPLING: clamp num_layers & seq_len ───────────────────────
        # Dipanggil setelah semua parameter lain diketahui (hidden_dim,
        # opt_type, batch_size, ffn_type, dll) sehingga kombinasi worst-case
        # tidak mungkin terjadi secara matematis.
        # Ini mencegah OOM dari awal — bukan reaktif seperti shrink di bawah.
        _seq_candidates = [s for s in [512, 1024, 2048, 4096, 8192, 16384]
                           if fp["seq_range"][0] <= s <= fp["seq_range"][1]] or [2048]
        num_layers, seq_len = self._budget_clamp(
            hidden_dim    = hidden_dim,
            num_heads     = num_heads,
            num_kv_heads  = kv_heads if attn_type in (
                AttentionType.GQA, AttentionType.MQA) else num_heads,
            head_dim      = head_dim,
            ffn_type      = ffn_type,
            ffn_mult      = ffn_mult,
            ...
            l_lo          = _l_lo,
            l_hi          = _l_hi,
            seq_candidates= _seq_candidates,
            rng           = rng,
        )
        # ── END BUDGET SAMPLING ───────────────────────────────────────────────
