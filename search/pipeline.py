...
def stage_generate(gpu: GPUSpec, families, n_per_family: int, seed: int):
    ...
    archs.sort(key=lambda x: (
        # Primary: fits_gpu dulu (yang muat GPU dapat prioritas)
        (0 if x.fits_gpu else 1),
        # Secondary: preliminary combined estimate
        # pakai fitness_score (hardware proxy) + vram_usage_pct (efficiency)
        # bukan fitness saja karena fitness tidak reliable untuk final ranking
...
def stage_training_nas(
    ...
    *,
    ...
    # Sort by preliminary combined estimate (hw + train seimbang)
    # bukan training_score saja — supaya RL resources tidak bias ke high-train arcs
    refined_archs.sort(
        key=lambda a: (
            hw_scores.get(a.arch_id, 0.0) * 0.5 +
            train_map.get(a.arch_id, 0.0) * 0.5
        ),
        reverse=True,
    )
...
def stage_balanced_scoring(
    ...
    # ── Best arch selection ───────────────────────────────────────────────────
    # Kriteria (berurutan):
    #   1. fits_gpu = True (wajib — OOM Guard sudah handle ini)
    #   2. Kedua skor hw DAN train minimal 0.70 (tidak boleh extreme di satu sisi)
    #   3. Combined score tertinggi dari kandidat yang lolos kriteria 1+2
    #   4. Fallback: combined score tertinggi tanpa threshold (jika semua < 0.70)

    MIN_SCORE_THRESHOLD = 0.70   # minimum hw DAN train untuk best arch

    # Kandidat tier 1: fits_gpu + kedua skor ≥ threshold
    tier1 = [
        a for a in archs_sorted
        if a.fits_gpu
        and hw_scores.get(a.arch_id, 0.0)    >= MIN_SCORE_THRESHOLD
        and train_scores.get(a.arch_id, 0.0) >= MIN_SCORE_THRESHOLD
    ]

    # Kandidat tier 2: fits_gpu saja (tanpa threshold)
    tier2 = [a for a in archs_sorted if a.fits_gpu]

    if tier1:
        best = tier1[0]   # combined tertinggi yang balanced
        tier = "balanced (hw≥0.70 & train≥0.70)"
    elif tier2:
        best = tier2[0]   # combined tertinggi yang fits_gpu
        tier = "fits_gpu only (threshold tidak tercapai)"
    else:
        # OOM Guard sudah handle — ini tidak seharusnya terjadi
        best = archs_sorted[0] if archs_sorted else None
        tier = "⚠ fallback (semua OOM — periksa OOM Guard)"

    if best is None:
        console.print("[bold red]  ❌ Tidak ada arc yang valid![/bold red]")
        return archs_sorted, combined_map, None

    pre_rank  = pre_nas_rank.get(best.arch_id, "?")
    hw_best   = hw_scores.get(best.arch_id, 0.0)
    tr_best   = train_scores.get(best.arch_id, 0.0)
    console.print(
        f"[bold green]★ Top Pick: {best.arch_id}  "
        f"[dim](pre-NAS rank: #{pre_rank} → post-NAS: #1 | {tier})[/dim]\n"
        f"  combined={combined_map.get(best.arch_id, 0):.5f}  "
        f"hw={hw_best:.4f}  train={tr_best:.4f}  "
